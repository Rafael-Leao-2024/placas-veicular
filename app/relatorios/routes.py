from flask import Blueprint, render_template, request, flash, session, Response, redirect, url_for
...
def diario():
    ...
    vendas = Venda.query.filter_by(loja_id=loja_id, ativo=True, data=data_ref).order_by(Venda.created_at.desc()).all() 
    
    total_vendas = sum(v.total for v in vendas)
    total_recebido = sum(v.total for v in vendas if v.pago)
    total_devendo = sum(v.total for v in vendas if not v.pago and v.forma_pagamento == 'fiado')
...
def exportar_diario():
    ...
    vendas = Venda.query.filter_by(loja_id=loja_id, ativo=True, data=data_ref).all()
    
    # Create CSV
    si = StringIO()
    writer = csv.writer(si)
    writer.writerow(['ID', 'Cliente', 'Vendedor', 'Total', 'Forma Pagamento', 'Pago', 'Data'])
    ...
    return Response(
        output,
        mimetype='text/csv',
        headers={'Content-Disposition': f'attachment; filename=relatorio_diario_{data_ref}.csv'}
    )
...
def exportar_mensal():
    ...
    return Response(
        output,
        mimetype='text/csv',
        headers={'Content-Disposition': f'attachment; filename=relatorio_mensal_{primeiro_dia.strftime("%B_%Y")}.csv'}
    )